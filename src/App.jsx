...
function App() {
  ...
  const [todo, setTodo] = useState("")   //created state to add a todo
  const [todos, setTodos] = useState([])   //Todos is an array whihc holds all the todo
  const [showFinished, setshowFinished] = useState(true)

  useEffect(() => {
    let todoString = localStorage.getItem("todos")
    if(todoString){
      let todos = JSON.parse(localStorage.getItem("todos"))  //"todos" in double quotes because it is an array
      setTodos(todos)
    }  
  }, [])
  ...
  const handleAdd = ()=>{
    setTodos([...todos, {id: uuidv4(), todo, isCompleted: false}])  //there are other todos that are already present and we have added our current todo which is not completed yet
    setTodo("")
    saveToLS()
  ...
  return (
    ...
    <div className="flex-container">
      <div className="new-div"><VRoidCharacter /></div>
      <div className = "mx-3 md:comtainer md:mx-auto my-5 rounded-xl p-5 bg-yellow-50 min-h-[80vh] md:w-[35%]">
...
function App() {
  ...
  return (
    <>
      ...
      <div className="flex-container">
        <div className="new-div"><VRoidCharacter /></div>
        <div className="mx-3 md:container md:mx-auto my-5 rounded-xl p-5 bg-yellow-50 min-h-[80vh] md:w-[35%]">
          <h1 className='font-bold text-center text-3xl'>iTask - Manage your tasks at one place</h1>
          <div className="addTodo my-5 flex flex-col gap-4">
            <h2 className='text-2xl font-bold'>Add a Todo</h2>
            <input onChange={handleChange} value={todo} type="text" className='w-full rounded-lg px-5 py-1' />
